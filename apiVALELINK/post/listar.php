...
// Verificando se há dados e retornando uma resposta mais clara
if (count($dados) > 0) {
    // Retorna os dados corretamente se houver itens
    $result = json_encode(array('success' => true, 'resultado' => $dados, 'totalItems' => count($dados) + $inicio));
} else {
    // Retorna um array vazio quando não há dados
    $result = json_encode(array('success' => false, 'resultado' => [], 'totalItems' => 0));
}
