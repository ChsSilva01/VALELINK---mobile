...
if (isset($_GET['cod_empresa'])) {
    ...
    $res = $query->fetchAll(PDO::FETCH_ASSOC);
        ...
        $quantidade_de_seguidores_empresa = $res[$i]['quantidade_de_seguidores_empresa'];
        $foto_empresa = $res[$i]['foto_empresa'];

            }

            if(count($res) > 0){
                    $result = json_encode(array('success'=>true, 'nome_empresa'=>$nome_empresa, 'descricao_da_empresa'=>$descricao_da_empresa, 'quantidade_de_funcionario_empresa'=>$quantidade_de_funcionarios, 'quantidade_de_seguidores_empresa'=>$quantidade_de_seguidores_empresa, 'foto_empresa'=>$foto_empresa));
